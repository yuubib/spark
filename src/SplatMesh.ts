...
const splatMeshWorldToViewScratch = new THREE.Matrix4();
const splatMeshObjectToWorldScratch = new THREE.Matrix4();
const splatMeshWorldToObjectScratch = new THREE.Matrix4();
const splatMeshViewToObjectScratch = new THREE.Matrix4();
...
export interface SplatMeshColorMatchSelectionOptions
  extends SplatMeshColorMatchOptions {
...
function normalizeColorMatchThreshold(threshold = 0): number {
  return Math.min(1, Math.max(0, Number.isFinite(threshold) ? threshold : 0));
}

function splatColorMatchesThreshold(
  color: SplatColorRaw,
  seed: SplatColorRaw,
  threshold: number,
): boolean {
  return (
    Math.abs(color.r - seed.r) <= threshold &&
    Math.abs(color.g - seed.g) <= threshold &&
    Math.abs(color.b - seed.b) <= threshold
  );
}
...
export class SplatMesh extends SplatGenerator {
  ...
  private scanSplatColorMatches(
    ...
    seed: SplatColorRaw,
    {
      ...
      mode = "all",
      maxMatches,
    }: Omit<SplatMeshColorMatchOptions, "seedIndex">,
    onMatch?: (index: number) => boolean | undefined,
  ): SplatMeshColorMatchSummary {
    ...
    const testColor = (index: number, r: number, g: number, b: number) => {
      ...
      matched += 1;
      if (onMatch?.(index) === false || matched >= max) {
        earlyExit = true;
        return false;
      }
      return true;
  ...
  selectSplatStateColorMatches({
    ...
    mutationOptions = {},
    ...matchOptions
  }: SplatMeshColorMatchSelectionOptions): SplatMeshColorMatchSelectionResult | null {
  ...
  selectSplatStateColorMatchesStream({
    ...
    mutationOptions = {},
    ...matchOptions
  }: SplatMeshColorMatchSelectionOptions): SplatMeshColorMatchStreamSelectionResult | null {
    const source = this.splats;
    const seed = { r: 0, g: 0, b: 0 };
    ...
    let match: SplatMeshColorMatchSummary | null = null;
    const mutation = this.mutateEditorState((state) =>
      state.selectCandidatesFromProducer(
        (consume) => {
  ...
  update({
    ...
    if (!this.covSplats) {
      ...
      const worldToView = splatMeshWorldToViewScratch
        .copy(viewToWorld)
        .invert();
      if (
      ...
      const worldToObject = splatMeshWorldToObjectScratch
        .copy(objectToWorld)
        .invert();
      const viewToObjectMatrix = splatMeshViewToObjectScratch
        .copy(worldToObject)
