...
import {
  ...
  SplatEditorState,
  type SplatEditorStateCandidateConsumer,
...
interface SplatScreenPickProjectionCache {
  readonly numSplats: number;
  readonly targetVersion: number;
  readonly viewportWidth: number;
  readonly viewportHeight: number;
  readonly objectToClipElements: readonly number[];
...
function resolveSplatScreenPickRankPoint(
  ...
  if (shape.kind === "point") {
    return {
      x: shape.x * viewportWidth,
      y: shape.y * viewportHeight,
    };
  }
  return {
...
function isBetterSplatScreenPickCenter(
  ...
  current: SplatScreenPickNearestCenterResult,
  rankMode: SplatScreenPickNearestCenterRankMode,
): boolean {
  switch (rankMode) {
    case "screen-distance-depth":
      return (
        compareNumber(candidate.screenDistanceSq, current.screenDistanceSq) <
          0 ||
        (nearlyEqual(candidate.screenDistanceSq, current.screenDistanceSq) &&
          compareNumber(candidate.ndcZ, current.ndcZ) < 0) ||
        (nearlyEqual(candidate.screenDistanceSq, current.screenDistanceSq) &&
          nearlyEqual(candidate.ndcZ, current.ndcZ) &&
    ...
    default:
      throw new Error(
        `Unsupported splat nearest center rank mode: ${rankMode}`,
      );
  }
}
...
function copyMatrixElements(elements: readonly number[]): number[] {
  return Array.from(elements);
}

function matrixElementsEqual(
  a: readonly number[],
  b: readonly number[],
): boolean {
...
const SPLAT_CENTER_INTERSECTION_AUTO_CPU_MAX_SPLATS = 1_250_000;
...
function shouldPreferGpuSplatCenterProcessorForSelectedFilter(
  numSplats: number,
  editorState: SplatEditorState | null | undefined,
  editorStateMode: NonNullable<SplatScreenPickOptions["editorStateMode"]>,
): boolean {
  ...
  if (
    !Number.isFinite(safeNumSplats) ||
    safeNumSplats <= 0 ||
    safeNumSplats > SPLAT_CENTER_INTERSECTION_AUTO_CPU_MAX_SPLATS
  ) {
    return false;
  }
  ...
  return (
    selectedCount > SPLAT_CENTER_INTERSECTION_AUTO_SELECTED_GPU_MIN_SPLATS &&
    selectedCount / safeNumSplats >
      SPLAT_CENTER_INTERSECTION_AUTO_SELECTED_GPU_MIN_RATIO
  );
}
...
export class SparkRenderer extends THREE.Mesh {
  ...
  private centerIntersectionTarget?: THREE.WebGLRenderTarget;
  private centerIntersectionPixels?: Uint8Array;
  private centerIntersectionMaterial?: THREE.RawShaderMaterial;
  private centerIntersectionQuad?: FullScreenQuad;
  private centerIntersectionMaskTexture?: THREE.DataTexture;
  private centerIntersectionMaskData?: Uint8Array;
  private centerIntersectionEmptyMaskTexture?: THREE.DataTexture;
  ...
  dispose() {
    ...
    if (this.centerIntersectionTarget) {
      ...
      this.centerIntersectionTarget = undefined;
    }
    if (this.centerIntersectionMaskTexture) {
      this.centerIntersectionMaskTexture.dispose();
      this.centerIntersectionMaskTexture = undefined;
      this.centerIntersectionMaskData = undefined;
    }
    if (this.centerIntersectionEmptyMaskTexture) {
      this.centerIntersectionEmptyMaskTexture.dispose();
      this.centerIntersectionEmptyMaskTexture = undefined;
    }
    this.centerIntersectionMaterial?.dispose();
    this.centerIntersectionMaterial = undefined;
    this.centerIntersectionQuad?.dispose();
    this.centerIntersectionQuad = undefined;
  ...
  private selectSplatStateFromScreenPickCpuProducer(
    options: SplatScreenPickStateSelectionOptions,
  ): SplatScreenPickStateSelectionResult | null {
    const totalStartedAt = readNowMs();
    const {
      scene,
      camera,
    ...
    const editorStateMode =
      options.editorStateMode ??
      editorSelectionOperationToPickFilterMode(operation);
    const collectStats = createSplatScreenPickCenterCollectStats();
    const requestedProcessor = options.centerProcessor ?? "auto";
    const editorState = target.getEditorState();
    const shouldUseCpuAuto =
    ...
    } satisfies SplatScreenPickStats;
  ...
  async pickNearestSplatCenterIndex(
    options: SplatScreenPickNearestCenterOptions,
  ): Promise<SplatScreenPickNearestCenterResult | null> {
    const totalStartedAt = readNowMs();
    const { scene, camera, target } = options;
    const size = this.renderer.getDrawingBufferSize(new THREE.Vector2());
    const width = Math.max(1, Math.floor(options.width ?? size.x));
    ...
    if (options.candidateMode && options.candidateMode !== "centers") {
      throw new Error(
        `Unsupported splat nearest center candidate mode: ${options.candidateMode}`,
      );
    }
    if (!(target instanceof SplatMesh) || !target.isInitialized) {
      return null;
    ...
    let cpuFallbackReason: SplatScreenPickCenterProcessorFallbackReason =
      requestedProcessor === "cpu"
        ...
        : shouldUseCpuAuto
          ...
          : "nearest-unsupported";
    if (!shouldUseCpuAuto && requestedProcessor !== "cpu") {
      const sourceSplatCount = target.splats?.getNumSplats?.();
      const targetSplatCount = normalizeSplatCount(target.numSplats);
      const sourceCountMatches =
        sourceSplatCount == null || sourceSplatCount === targetSplatCount;
      const finiteMax =
        options.maxCandidates != null &&
        Number.isFinite(Math.floor(options.maxCandidates));
      if (selectedTransform) {
        ...
        if ("indices" in gpuResult) {
          ...
          const hit = this.rankNearestSplatScreenPickCenterIndices({
            target,
            camera,
            shape: options.shape,
            rect,
          ...
          collectStats.uniqueHitCount = hit ? 1 : 0;
          options.onStats?.({
            shapeKind: options.shape.kind,
            candidateMode: "centers",
            renderMode,
            ...
            readRect: layout.readRect,
            pixelHitCount: 0,
            mappedHitCount: hit ? 1 : 0,
            sourceStableHitCount: hit ? 1 : 0,
            collect: {
              pixelCount: 0,
              candidatePixelCount: 0,
              maskTestedPixelCount: 0,
  ...
  private forEachSplatScreenPickTargetCenterRaw({
    ...
    const emitCenter = (
      ...
      if (selectedTransform && bits === SPLAT_EDITOR_STATE_SELECTED) {
        ...
        return callback(index, center.x, center.y, center.z, bits);
      }
      return callback(index, centerX, centerY, centerZ, bits);
    };
    if (
      editorState &&
      !skipEditorStateFilter &&
      ...
      editorState.forEachSelectedIndex((index, bits) => {
        ...
        if (emitCenter(index, center.x, center.y, center.z, bits) === false) {
          return false;
        }
      });
      return;
    ...
    target.forEachSplatCenterRaw((index, centerX, centerY, centerZ) => {
      ...
      return emitCenter(index, centerX, centerY, centerZ, bits);
    });
  }
  ...
  private getSplatScreenPickProjectionCache({
    ...
    this.centerProjectionCache ??= new WeakMap<
      ...
      SplatScreenPickProjectionCache
    >();
    const cached = this.centerProjectionCache.get(target);
    if (
      cached &&
      cached.numSplats === numSplats &&
      cached.targetVersion === target.version &&
    ...
    const projectedCenters =
      cached && cached.projectedCenters.length >= numSplats * 3
    ...
    const nextCache: SplatScreenPickProjectionCache = {
      ...
      viewportWidth,
      viewportHeight,
      objectToClipElements: copyMatrixElements(objectToClipElements),
      projectedCenters,
    };
  ...
  private collectSplatScreenPickCenterIndices({
    ...
    stats: SplatScreenPickCenterCollectStats;
    indexBuffer?: SplatScreenPickIndexBuffer;
    consumeIndex?: SplatEditorStateCandidateConsumer;
  }): Uint32Array {
    const max =
      maxCandidates != null
        ? Math.max(0, Math.floor(maxCandidates))
    ...
    scene.traverseVisible((object) => {
      ...
      const shouldUseSelectedIndexPath =
        editorState &&
        !skipEditorStateFilter &&
        target.hasIndexedSplatCenters() &&
        canUseSelectedSplatCenterIndexMode(editorStateMode);
  ...
  private rankNearestSplatScreenPickCenterIndices({
    ...
    indices: Uint32Array;
    stats: SplatScreenPickCenterCollectStats;
  }): SplatScreenPickNearestCenterResult | null {
    if (indices.length === 0) {
      return null;
    }
    ...
    let best: SplatScreenPickNearestCenterResult | null = null;
  ...
  private ensureSplatCenterIntersectionTarget(width: number, height: number) {
  ...
  private ensureSplatCenterIntersectionMaskTexture(
    ...
    if (mask.data instanceof Uint8Array && mask.data.length >= byteLength) {
      ...
      if (
        !this.centerIntersectionMaskData ||
        this.centerIntersectionMaskData.length < byteLength
      ) {
    ...
    texture.needsUpdate = true;
    this.centerIntersectionMaskTexture = texture;
    return texture;
  }

  private ensureSplatCenterIntersectionMaterial(): THREE.RawShaderMaterial {
  ...
  private async renderSplatCenterIntersectionPass(
    ...
    if (
      !this.centerIntersectionPixels ||
      this.centerIntersectionPixels.length < byteLength
    ) {
  ...
  private collectNearestSplatScreenPickCenterIndex({
    ...
    scene.traverseVisible((object) => {
      if (candidateCount >= max || object !== target) {
        return;
      }
