...
export class PackedSplats implements SplatSource {
  ...
  setSplats(writes: PackedSplatWriteBuffers) {
    ...
    const canMarkSourceLayers =
      !!source &&
      !!sourceImage &&
      sourceImage.data === packedSplats &&
      sourceImage.width * sourceImage.height > 0;
    const sourceSplatsPerLayer = canMarkSourceLayers
    ...
    const centerTextureImage = centerTexture?.image;
    const canPatchCenterTexture =
      !!centerTexture &&
      !!centerTextureData &&
      !!centerTextureImage &&
      !this.centerMatchTextureNeedsUpdate &&
      centerTextureImage.data === centerTextureData &&
      centerTextureImage.width * centerTextureImage.height > 0;
    const centerTextureSplatsPerLayer = canPatchCenterTexture
      ? centerTextureImage.width * centerTextureImage.height
      : 0;
    const centerTextureDepth = canPatchCenterTexture
    ...
    for (let row = 0; row < count; row++) {
      ...
      if (centerMatchXyz) {
        ...
        if (canPatchCenterTexture) {
          ...
          if (
            textureOffset + 3 < centerTextureData.length &&
            layer >= 0 &&
            layer < centerTextureDepth
          ) {
